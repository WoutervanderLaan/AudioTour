...
export type CheckboxProps = TestProps<'Checkbox'> & {
  ...
  required?: boolean
  /** Whether the checkbox has an error (for styling) */
  hasError?: boolean
...
const CheckboxLabel = ({
  ...
  <Text.Label
    testId={`${testId}LabelText` as `${string}Text`}
    nativeID={labelId}
    ...
    {!!required && (
      <Text.Label
        testId={`${testId}RequiredText` as `${string}Text`}
        color="warning"
...
const CheckboxBox = ({
  ...
  <Box
    testId={`${testId}BoxView` as `${string}View`}
    nativeID={checkboxId}
    center
    style={[
      styles.checkboxBox,
