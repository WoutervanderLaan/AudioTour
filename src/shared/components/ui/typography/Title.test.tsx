...
describe('Title', () => {
  describe('Rendering', () => {
    it('should render children correctly', () => {
      render(<Title testID="TitleText">Title Text</Title>)
      expect(screen.getByText('Title Text')).toBeTruthy()
    })

    it('should render with default h1 level', () => {
      const {getByText} = render(
        <Title testID="TitleText">Default Title</Title>,
      )
    ...
    it('should render with default headingBold font family', () => {
      render(<Title testID="TitleText">Bold Title</Title>)
  ...
  describe('Accessibility', () => {
    it('should have accessibility role header', () => {
      const {getByText} = render(
        <Title testID="TitleText">Accessible Title</Title>,
      )
    ...
    it('should be accessible by default', () => {
      const {getByText} = render(
        <Title testID="TitleText">Accessible by default</Title>,
      )
  ...
  describe('Props Forwarding', () => {
    it('should forward testID prop', () => {
      render(<Title testID="TitleText">Test Title</Title>)
      expect(screen.getByTestId('TitleText')).toBeTruthy()
    })
  ...
  describe('Edge Cases', () => {
    it('should render with empty children', () => {
      const {root} = render(<Title testID="TitleText" />)
      expect(root).toBeTruthy()
    })

    it('should render with number children', () => {
      render(<Title testID="TitleText">{42}</Title>)
      expect(screen.getByText('42')).toBeTruthy()
    })

    it('should render with mixed content', () => {
      render(
        <Title testID="Title1Text">
          Part 1{' '}
          <Title
            testID="Title2Text"
            level="h2">
      ...
      expect(screen.getByTestId('Title1Text')).toHaveStyle({fontSize: 32})
      expect(screen.getByTestId('Title1Text')).toBeOnTheScreen()
      expect(screen.getByTestId('Title2Text')).toHaveStyle({fontSize: 28})
      expect(screen.getByTestId('Title2Text')).toBeOnTheScreen()
    })
  })
})
