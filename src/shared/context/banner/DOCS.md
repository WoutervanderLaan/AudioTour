...
1. **One banner at a time**: The system shows only one banner at a time. Calling \`showBanner()\` multiple times will replace the current banner.

2. **Clean up on unmount**: If a component shows a banner conditionally, use \`useEffect\` cleanup to hide the banner when conditions change.

3. **Handle dismiss**: Always provide an \`onDismiss\` handler to give users a way to close the banner.
...
5. **Keep messages concise**: Banner messages should be brief and actionable.
